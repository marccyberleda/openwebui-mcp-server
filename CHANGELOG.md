...
### Fixed
- `openwebui_get_knowledge` now correctly returns the file list. The OWU API returns `files: null` on the knowledge detail endpoint; the tool now fetches files from the `/api/v1/knowledge/{id}/files` sub-endpoint and displays `filename` for each file.
